...
export default function DataSatuanPage() {
  ...
  const fetchKategori = async (page = 1, searchTerm = "") => {
    try {
      ...
      // Ubah limit menjadi 5 (bukan 10)
      const response = await fetch(
        `http://localhost:3200/api/kategori?page=${page}&limit=${pagination.itemsPerPage}&search=${searchTerm}`,
        {
          headers: {
      ...
      if (response.ok) {
        // Pastikan hanya mengambil maksimal 5 data
        const limitedData = result.data.slice(0, pagination.itemsPerPage);
        setData(limitedData);
        
        // Hitung ulang pagination berdasarkan data yang terbatas
            ...
            {loading ? (
              ...
              <>
                <table className="w-full border-collapse text-x1">
                  ...
                  <tbody>
                    {data.slice(0, 5).map((row, index) => ( // Pastikan hanya 5 data yang ditampilkan
                      <tr
                        key={row.id}
                ...
                <div className="flex justify-between items-center px-6 py-4 bg-white border-t">
                  <div className="text-sm text-gray-600">
                    Menampilkan {Math.min(data.length, 5)} dari {pagination.totalItems} data
                  </div>
